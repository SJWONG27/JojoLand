...
public class UndirectedGraph<T extends Comparable<T>, N extends Comparable<N>> {
    Vertex<T,N> head;
    int size;
            ...
            if(sourceVertex.vertexInfo.compareTo(source) ==0){
                while(destinationVertex!=null){
                    if(destinationVertex.vertexInfo.compareTo(destination) == 0){
                        Edge<T,N> currentSourceEdge = sourceVertex.firstEdge;
                        Edge<T,N> newSourceEdge = new Edge<>(destinationVertex, w, currentSourceEdge);
                        sourceVertex.firstEdge= newSourceEdge;
                        sourceVertex.outdeg++;
                        sourceVertex.indeg++;
                        Edge<T,N> currentDestEdge = destinationVertex.firstEdge;
                        Edge<T,N> newDestEdge = new Edge<>(sourceVertex, w, currentDestEdge);
    ...
    public ArrayList<String> getNeighboursWithIndex(T v) {
        ArrayList<T> neighbours = getNeighbours(v);
        if (neighbours == null) return null;
    
        ArrayList<String> indexedNeighbours = new ArrayList<>();
        for (int i = 0; i < neighbours.size(); i++) {
            String indexedNeighbour = String.format("[%s] %s", (char) ('A' + i), neighbours.get(i).toString());
            indexedNeighbours.add(indexedNeighbour);
        }
        return indexedNeighbours;
    }    
